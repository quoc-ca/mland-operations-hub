...
| Stock Reservation | Belongs to a paid retail order and reserves exactly one ready-ring unit. Expiry and stock model are `TBD`. |
| Delivery Handoff | Belongs to a retail order only when the Member selects delivery; it records the minimum handoff evidence. Retention and fields are `TBD`. |
